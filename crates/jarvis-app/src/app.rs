        ...
        match vad_state {
            ...
            VadState::VoiceActive => {
                ...
                if let Some(mut recognized_voice) = stt::recognize(frame_buffer, false) {
                    ...
                    let short_phrase = recognized_voice.chars().count() < 4
                        && !is_weather_phrase(&recognized_voice);
                    if short_phrase {
...
fn process_text_command(text: &str, rt: &tokio::runtime::Runtime) {
    ...
    let filtered = jarvis_core::speech_normalize::normalize(filtered.trim());
...
fn is_weather_phrase(text: &str) -> bool {
    text.contains("погод")
}

fn finish_command_execution(
    cmd_path: &std::path::PathBuf,
    cmd_config: &commands::JCommand,
    text: &str,
    extracted_slots: Option<std::collections::HashMap<String, commands::SlotValue>>,
) -> bool {
    match commands::execute_command(cmd_path, cmd_config, Some(text), extracted_slots.as_ref()) {
        Ok(chain) => {
            info!("Command executed successfully");

            if cmd_config.cmd_type == "terminate" {
                voices::play_shutdown_response(
...
fn execute_command(text: &str, rt: &tokio::runtime::Runtime) -> bool {
    let text = jarvis_core::speech_normalize::normalize(text);
    ...
    if let Some((cmd_path, cmd_config)) = cmd_result {
        ...
        return finish_command_execution(&cmd_path, &cmd_config, &text, extracted_slots);
    } else {
        info!("No command found for: {}", text);
        voices::play_not_found();
        ipc::send(IpcEvent::Error {
            message: format!("Команда не найдена: {}", text),
        });
    }
...
pub fn close(code: i32) {
    ...
    voices::play_goodbye();
    quit_entire_application_with_code(code);
}

/// After shutdown audio: close GUI, then exit jarvis-app (tray + voice).
fn quit_entire_application() {
    quit_entire_application_with_code(0);
}

pub fn quit_entire_application_with_code(code: i32) {
    ipc::send(IpcEvent::QuitApplication);
    ipc::send(IpcEvent::Stopping);
    std::thread::sleep(std::time::Duration::from_millis(450));
    info!("Exiting jarvis-app (code {}).", code);
