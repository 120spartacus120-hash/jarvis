...
// include core
use jarvis_core::{
    audio, audio_processing, commands, config, custom_commands, db, listener, recorder, stt, intent,
    ipc::{self, IpcAction},
    i18n, voices, models,
    set_commands_list, APP_CONFIG_DIR, APP_LOG_DIR, COMMANDS_LIST, DB,
};
...
fn main() -> Result<(), String> {
    ...
    let cmds = custom_commands::reload_commands_list();
    info!(
        "Commands initialized. Count: {}, phrases: {}",
        cmds.iter().map(|c| c.commands.len()).sum::<usize>(),
        custom_commands::load().thanks_phrases.len()
            + custom_commands::load().shutdown_phrases.len()
    );
