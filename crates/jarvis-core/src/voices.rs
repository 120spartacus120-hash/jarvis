...
pub fn play_shutdown_response(sounds: &[String]) {
    if !play_all_from_list_blocking(sounds) {
        if !play_reaction_blocking(structs::Reaction::Goodbye) {
            let _ = play_reaction_blocking(structs::Reaction::Ok);
        }
    }

    // Let the last sample finish on the output device
    std::thread::sleep(Duration::from_millis(350));
}

/// Play every listed sound in order (blocking) — for shutdown so the full reply is heard.
fn play_all_from_list_blocking(sounds: &[String]) -> bool {
    if sounds.is_empty() {
        return false;
    }
    ...
    for sound_name in sounds {
        if let Some(path) = find_sound_file(&voice.path, &lang, sound_name) {
            debug!("Playing shutdown (blocking): {:?}", path);
            audio::play_sound_blocking(&path);
            played = true;
        } else {
            warn!("Shutdown sound not found: {} (lang: {})", sound_name, lang);
        }
    }
...
pub fn play_random_from_blocking(sounds: &[String]) -> bool {
    let voice = match get_current_voice() {
        Some(v) => v,
        None => {
            warn!("No current voice set");
    ...
    play_random_from_list_blocking(&voice.path, &get_current_language(), sounds)
}

fn play_random_from_list_blocking(voice_path: &Path, lang: &str, sounds: &[String]) -> bool {
    if sounds.is_empty() {
        return false;
    }
    ...
    for index in order {
        let sound_name = &sounds[index];
        if let Some(path) = find_sound_file(voice_path, lang, sound_name) {
...
fn play_reaction_blocking(reaction: structs::Reaction) -> bool {
    let voice = match get_current_voice() {
        Some(v) => v,
        None => return false,
    };
    ...
    let sounds = match reaction {
        structs::Reaction::Greet => &reactions.greet,
        structs::Reaction::Reply => &reactions.reply,
        structs::Reaction::Ok => &reactions.ok,
        structs::Reaction::NotFound => &reactions.not_found,
        structs::Reaction::Thanks => &reactions.thanks,
    ...
    play_random_from_list_blocking(&voice.path, &lang, sounds)
}
