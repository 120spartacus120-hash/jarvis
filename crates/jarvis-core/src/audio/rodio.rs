...
pub fn play_sound(filename: &PathBuf, sleep: bool) {
    play_sound_with_gain(filename, sleep, 1.0);
}

pub fn play_sound_with_gain(filename: &PathBuf, sleep: bool, gain: f32) {
    let file = BufReader::new(File::open(&filename).unwrap());
    let source = Decoder::new(file).unwrap().amplify(gain.max(0.0));
