...
settings-language-desc = Меню, кнопки та сповіщення програми.
settings-language-restart-note = Після зміни мови програма перезапуститься автоматично.
settings-tab-weather = Погода

# UPDATES
update-available = Доступне оновлення { $version }
...
commands-builtin = Вбудована
commands-card-greeting = Привітання
commands-card-greeting-desc = JARVIS відповість однією з фраз привітання.
commands-greeting-desc = Додайте фрази привітання — JARVIS відповість випадковою реплікою.
commands-greeting-hint = Скажіть «Джарвіс», потім одну з цих фраз.
commands-phrase-placeholder-greeting = Наприклад: привіт
