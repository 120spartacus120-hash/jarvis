# ### APP INFO
app-name = JARVIS
app-description = Sprachassistent
...
settings-language-desc = Menüs, Schaltflächen und Benachrichtigungen der App.
settings-language-restart-note = Nach dem Wechsel startet die App automatisch neu.
settings-tab-weather = Wetter
...
commands-card-greeting = Greeting
commands-card-greeting-desc = JARVIS replies with a greeting voice line.
commands-greeting-desc = Add greeting phrases — JARVIS will play a random greeting clip.
commands-greeting-hint = Say «Jarvis», then one of these phrases.
commands-phrase-placeholder-greeting = For example: hello
