...
1. Перейдите на страницу [Releases](https://github.com/120spartacus120-hash/jarvis/releases/latest) и скачайте **`JARVIS_*_x64-setup.exe`** (мастер установки NSIS).
2. Если Windows показывает предупреждение SmartScreen — **Подробнее** → **Выполнить в любом случае**. Это из‑за отсутствия платной подписи издателя, не из‑за вируса. Подробнее: [docs/УСТАНОВКА_И_ДОВЕРИЕ_WINDOWS.md](docs/УСТАНОВКА_И_ДОВЕРИЕ_WINDOWS.md).
3. Запустите установщик и следуйте инструкциям (русский мастер, ярлык на рабочем столе).
4. Программа автоматически проверяет обновления на GitHub при запуске (баннер «Обновить» / «Позже»).
